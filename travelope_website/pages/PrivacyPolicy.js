...
function PolicyTemplate(props) {
	return (
		...
		>{props.title}</BaseText>
			<BaseText $padding="0 0 0 40px"
			          $lineHeight="23px" $letterSpacing="1px" $fontFamily="Noto Sans TC" $fontSize={15}
			          $fontWeight={300}>
				{props.content}
			</BaseText>
		</VStack>)
			...
			<BaseGrid>
				...
				<SiteBody>
					<VStack>
						...
						<PolicyTemplate initial={{x: -100, opacity: 0}}
						                animate={{x: 0, opacity: 1}}
						                transition={{delay: 1}} title="五、與第三人共用資料之政策" content="本網站絕不會提供、交換、出租或出售任何您的個人資料給其他個人、團體、私人企業或公務機關，但有法律依據或合約義務者，不在此限。
							前項但書之情形包括不限於：
							一、經由您同意。
							二、法律明文規定。
						...
						<PolicyTemplate initial={{x: -100, opacity: 0}}
						                animate={{x: 0, opacity: 1}}
						                transition={{delay: 1.1}} title="六、資料存放與使用"
